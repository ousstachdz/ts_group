...
class SaleOrder(models.Model):
    ...
    def write(self,vals):
        authorized_users = [
            self.user_id.sale_team_id.user_id,
            self.create_uid,
            self.user_id
        ]
        current_user = self.env['res.users'].browse(self.env.uid)
        if current_user not in authorized_users and not current_user.has_group('sales_team.group_sale_manager'):
...
class SaleOrderLine(models.Model):
    ...
    def _timesheet_service_generation(self):
        ...
        def _determine_project(so_line):
            ...
            return False
        def send_mail(project):
            template = self.env.ref('abs_greenskay_process_nv_nv.new_task_project_notification')
            template.with_context(name=project.name, id= project.id).send_mail(project.sale_line_id.id, force_send=True)
